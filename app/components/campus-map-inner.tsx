...
function SpreadScheduledMarkersOnExport({
  ...
  useEffect(() => {
    ...
    const MIN_SEPARATION = 22; // slightly larger than the 18px icon
    const ITERATIONS = 12;

    // Wait for FitBoundsOnExport to settle, then run a pixel-space
    // repulsion pass and update each marker's icon with an anchor offset.
    const apply = () => {
      ...
      for (let iter = 0; iter < ITERATIONS; iter++) {
        ...
        for (let a = 0; a < entries.length; a++) {
          for (let b = a + 1; b < entries.length; b++) {
            ...
            if (dist === 0) {
              // Identical positions: push along a deterministic axis
              const angle = (a + 1) * 0.8;
    ...
    // Run once after the current event loop so FitBoundsOnExport's setView
    // has taken effect, and again on moveend in case the view is still
    // animating.
    ...
    return () => {
      ...
      // Restore default (zero-offset) icons on the live map.
